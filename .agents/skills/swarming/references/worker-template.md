...
Use this template verbatim when spawning a worker subagent. Fill in all `<placeholder>` values from the execution-plan.md and your swarm state.
...
```
Subagent(
  identity="Worker: <TRACK_NAME> — Wave <WAVE_NUMBER>",
  context="""
<WORKER_PROMPT>
"""
)
...
`Subagent(...)` is canonical architecture wording, not a requirement to use a literal API named `Subagent`. Replace it with the spawn primitive your current runtime exposes while preserving the same behavior.
...
## Context Boundary
You are a bounded worker subagent. Start with the task-specific context in this prompt, and only request or inherit broader parent context when it is genuinely necessary to complete the assigned work.
